...
class DensePoseResultsVisualizer:
    ...
    def visualize_separate(
        ...
        mask_images = []
        for i, result in enumerate(results):
            context = copy.deepcopy(self.create_visualization_context(image_bgr))
            boxes_xywh, labels, uv = result
            iuv_array = torch.cat(
                (labels[None].type(torch.float32), uv * 255.0)
            ).type(torch.uint8)
            mask_image = self.visualize_iuv_arr(context, iuv_array.cpu().numpy(), boxes_xywh)
...
class DensePoseMaskedColormapResultsVisualizer(DensePoseResultsVisualizer):
    ...
    def visualize_iuv_arr(self, context, iuv_arr: np.ndarray, bbox_xywh) -> None:
        ...
        mask = np.zeros(matrix.shape, dtype=np.uint8)
        mask[segm > 0] = 1
        image_bgr = self.mask_visualizer.visualize(context, mask, matrix, bbox_xywh)
        return image_bgr
